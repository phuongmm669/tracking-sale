...
const STRAPI_TOKEN = ''.trim();
...
async function createImportLog() {
  try {
    ...
    console.log(`📝 Đã tạo phiên Import mới: ${logData.fileName} (ID: ${logId})`);
    return logId;
  } catch (error) {
...
async function importOrders() {
  ...
  const parser = fs.createReadStream('./orders.csv').pipe(parse({ 
    columns: true, trim: true, bom: true 
  }));
  ...
  for await (const row of parser) {
    ...
    try {
      ...
      const orderData = {
        ...
        orderPrice: parseFloat((row['Giá trị đơn hàng (₫)'] || "0").replace(/[^0-9.-]+/g,"")) || 0,
        subId: row['Sub_id1'] || "",
        commission: parseFloat((row['Hoa hồng ròng tiếp thị liên kết(₫)'] || "0").replace(/[^0-9.-]+/g,"")) || 0,
  ...
  console.log(`💡 Mẹo: Vào Strapi lọc import_log ID = ${currentImportLogId} để kiểm tra.`);
  console.log('-----------------------------------\n');
}
